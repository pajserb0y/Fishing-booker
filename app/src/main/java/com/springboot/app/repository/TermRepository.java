...
public interface TermRepository extends JpaRepository<Term, Integer> {
     ...
     @Query("SELECT r.weekendHouse.id FROM Term r WHERE (cast(:start as timestamp) BETWEEN r.startDateTime AND r.endDateTime) " +
...
//             "OR (cast(:end as timestamp) NOT BETWEEN r.startDateTime AND r.endDateTime)")
     List<Integer> findAllHouseIdsThatWorkForPeriod(@Param("start") Date start, @Param("end") Date end);
}
